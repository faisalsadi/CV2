...
for j in range(1,6):
    image1 = cv2.imread(f"Data/set_{j}/im_left.jpg")
    K = np.array([[576, 0, 511.5], [0, 576, 217.5], [0, 0, 1]], dtype=np.float32)
    depth_map = np.loadtxt('Data/example/depth_left.txt',delimiter=',')
    depth_matrix = np.loadtxt(f"Data/set_{j}/depth_left.txt",delimiter=',')
    K1 = np.array([[focals[j], 0, 511.5], [0, focals[j], 217.5], [0, 0, 1]], dtype=np.float32)

    # points_3d = reproject_to_3d(image, K, tt)
    # for i in range (11):
